...
use uc_db::repos::{staging, table};
use uc_errors::UcError;
use uc_openapi::delta::{
    DeltaCredentialOperation, DeltaCredentialsResponse, DeltaStorageCredential,
};
...
pub async fn get_table_credentials(
    ...
    let schema_row = uc_db::repos::schema::get_by_full_name(&state.pool, &catalog, &schema).await?;
    ...
    let storage_creds = if let Some(aws) = creds.aws_temp_credentials {
        vec![DeltaStorageCredential {
            prefix: url,
            operation: DeltaCredentialOperation::ReadWrite,
    ...
    Ok(Json(DeltaCredentialsResponse {
...
pub async fn get_staging_credentials(
    ...
    let uid: Uuid = table_id
        .parse()
        .map_err(|_| UcError::invalid_argument("invalid table_id"))?;
    let staging = staging::get_by_id(&state.pool, uid).await?;
    let scheme = UriScheme::from_url(&staging.staging_location);
    let ctx = CredentialContext {
        scheme,
        locations: vec![staging.staging_location.clone()],
        operation: CredentialOperation::ReadWrite,
        table_id: Some(uid),
        credential_json: None,
        role_arn: None,
    ...
    let storage_creds = if let Some(aws) = creds.aws_temp_credentials {
        vec![DeltaStorageCredential {
            prefix: staging.staging_location.clone(),
            operation: DeltaCredentialOperation::ReadWrite,
            config: Some(serde_json::json!({
                "awsAccessKey": aws.access_key_id,
                "awsSecretKey": aws.secret_access_key,
