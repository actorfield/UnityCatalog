...
use uc_db::repos::{catalog, property, schema};
...
pub async fn create(
    ...
    let catalog = catalog::get_by_name(&state.pool, &req.catalog_name).await?;
    ...
    if let Some(ref props) = req.properties {
        property::replace(&state.pool, id, "schema", props).await?;
    }
    if state.auth_enabled {
    ...
    let props = property::get_for_entity(&state.pool, id, "schema")
        .await
        .ok();
...
pub async fn list(
    ...
    let schemas = rows
        ...
        .map(|r| to_schema_info(r, catalog.name.clone(), None))
        .collect();
...
pub async fn get(
    ...
    let row = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let props = property::get_for_entity(&state.pool, row.id, "schema")
        .await
        .ok();
...
pub async fn update(
    ...
    let (cat, sch) = split2(&full_name)?;
    let existing = schema::get_by_full_name(&state.pool, cat, sch).await?;
    if state.auth_enabled {
        let user = get_user(&state, &claims.sub).await?;
        require(&state, user.id, existing.id, Privilege::Owner).await?;
    }
    let now = chrono::Utc::now().timestamp_millis();
    let row = schema::update(
        &state.pool,
        existing.id,
    ...
    if let Some(ref props) = req.properties {
        if !props.is_empty() {
            property::replace(&state.pool, row.id, "schema", props).await?;
        }
    }
    let props = property::get_for_entity(&state.pool, row.id, "schema")
        .await
...
pub async fn delete(
    ...
    let (tables, _) = table::list(&state.pool, existing.id, None, 1).await?;
    let (volumes, _) = volume::list(&state.pool, existing.id, None, 1).await?;
    let (funcs, _) = function::list(&state.pool, existing.id, None, 1).await?;
    let (models, _) = model::list_models(&state.pool, existing.id, None, 1).await?;

    let has_children =
        !tables.is_empty() || !volumes.is_empty() || !funcs.is_empty() || !models.is_empty();
    if has_children {
        if !force {
            return Err(UcError::new(
                uc_errors::ErrorCode::FailedPrecondition,
                format!(
                    "Schema '{}' is not empty. Use force=true to force deletion.",
                    full_name
                ),
            ));
        }
        // force=true: delete all children
        let (all_tables, _) = table::list(&state.pool, existing.id, None, 10000).await?;
        for t in all_tables {
            table::delete_columns(&state.pool, t.id).await?;
            property::delete_for_entity(&state.pool, t.id, "table").await?;
            table::delete(&state.pool, t.id).await?;
        }
        let (all_volumes, _) = volume::list(&state.pool, existing.id, None, 10000).await?;
        for v in all_volumes {
            volume::delete(&state.pool, v.id).await?;
        }
        let (all_funcs, _) = function::list(&state.pool, existing.id, None, 10000).await?;
        for f in all_funcs {
            function::delete(&state.pool, f.id).await?;
        }
        let (all_models, _) = model::list_models(&state.pool, existing.id, None, 10000).await?;
        for m in all_models {
            model::delete_model(&state.pool, m.id).await?;
        }
    }

    property::delete_for_entity(&state.pool, existing.id, "schema").await?;
    state
        .authorizer
        .remove_hierarchy_children(existing.id)
        .await?;
    schema::delete(&state.pool, existing.id).await?;
    Ok(StatusCode::OK)
}

fn to_schema_info(
    r: uc_db::models::schema::SchemaRow,
    catalog_name: String,
