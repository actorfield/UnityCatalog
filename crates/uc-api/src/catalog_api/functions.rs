...
use uc_db::{
    models::function::{FunctionParamRow, FunctionRow},
    repos::{function, property, schema},
};
use uc_errors::UcError;
use uc_openapi::catalog::{
    ColumnTypeName, CreateFunctionRequest, FunctionInfo, FunctionParameterInfo,
    FunctionParameterInfos, ListFunctionsResponse,
};
...
pub async fn create(
...
) -> Result<Json<FunctionInfo>, UcError> {
    let fi = req.function_info;
    let schema = schema::get_by_full_name(&state.pool, &fi.catalog_name, &fi.schema_name).await?;
    ...
    let row = FunctionRow {
        ...
        full_data_type: fi.full_data_type.clone(),
        external_language: fi.external_language.clone(),
        is_deterministic: fi.is_deterministic,
        is_null_call: fi.is_null_call,
        parameter_style: fi.parameter_style.clone(),
        routine_body: fi.routine_body.clone(),
        routine_definition: fi.routine_definition.clone(),
        sql_data_access: fi.sql_data_access.clone(),
        security_type: fi.security_type.clone(),
        specific_name: fi.specific_name.clone(),
    };
    let created = function::create(&state.pool, &row).await?;
    // Insert input params
    if let Some(ref ip) = fi.input_params {
        if let Some(ref params) = ip.parameters {
            let rows: Vec<FunctionParamRow> = params
                .iter()
                .enumerate()
                .map(|(i, p)| FunctionParamRow {
                    ...
                    type_name: p
                        ...
                        .as_ref()
                        .map(|t| format!("{:?}", t).to_uppercase()),
                    type_precision: p.type_precision,
                    type_scale: p.type_scale,
                    type_interval_type: p.type_interval_type.clone(),
                    comment: p.comment.clone(),
                    parameter_mode: p.parameter_mode.clone(),
                    parameter_default: p.parameter_default.clone(),
                })
                .collect();
            function::insert_params(&state.pool, &rows).await?;
        }
    }
    if let Some(ref props) = fi.properties {
        property::replace(&state.pool, id, "function", props).await?;
    }
    if state.auth_enabled {
    ...
    let (input, _ret) = function::get_params(&state.pool, id).await?;
    let props = property::get_for_entity(&state.pool, id, "function")
        .await
        .ok();
    Ok(Json(to_function_info(
        created,
        &fi.catalog_name,
        &fi.schema_name,
        input,
...
pub async fn list(
    ...
    let functions = rows
        ...
        .map(|r| to_function_info(r, &params.catalog_name, &params.schema_name, vec![], None))
        .collect();
...
pub async fn get(
    ...
    let row = function::get_by_schema_and_name(&state.pool, schema.id, func).await?;
    let (input, _ret) = function::get_params(&state.pool, row.id).await?;
    let props = property::get_for_entity(&state.pool, row.id, "function")
        .await
        .ok();
...
pub async fn delete(
    ...
    let (cat, sch, func) = split3(&full_name)?;
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let existing = function::get_by_schema_and_name(&state.pool, schema.id, func).await?;
    if state.auth_enabled {
        let user = get_user(&state, &claims.sub).await?;
        require(&state, user.id, existing.id, Privilege::Owner).await?;
    }
    property::delete_for_entity(&state.pool, existing.id, "function").await?;
    function::delete(&state.pool, existing.id).await?;
    Ok(StatusCode::OK)
}
...
fn to_function_info(
    ...
    cat: &str,
    sch: &str,
    input: Vec<FunctionParamRow>,
    props: Option<std::collections::HashMap<String, String>>,
) -> FunctionInfo {
    let params: Vec<FunctionParameterInfo> = input
        .into_iter()
        .map(|p| {
            ...
            FunctionParameterInfo {
                ...
                type_json: p.type_json,
                type_name,
                type_precision: p.type_precision,
                type_scale: p.type_scale,
                type_interval_type: p.type_interval_type,
                position: Some(p.ordinal_position),
                parameter_type: p.parameter_mode.as_ref().map(|_| "PARAM".to_string()),
    ...
    FunctionInfo {
        ...
        input_params: Some(FunctionParameterInfos {
            parameters: Some(params),
        }),
        return_params: None,
        data_type: None,
        full_data_type: r.full_data_type,
        routine_body: r.routine_body,
        routine_definition: r.routine_definition,
        parameter_style: r.parameter_style,
        is_deterministic: r.is_deterministic,
        sql_data_access: r.sql_data_access,
        is_null_call: r.is_null_call,
