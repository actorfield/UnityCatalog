...
use uc_db::{models::delta::DeltaCommitRow, repos::{DeltaCommitRepo, SchemaRepo, TableRepo}};
...
pub async fn commit(State(state): State<AppState>, Json(req): Json<CommitRequest>) -> Result<Json<GetCommitsResponse>, UcError> {
    ...
    let row = DeltaCommitRow {
        id: Uuid::new_v4(), table_id: table.id,
        commit_version: req.version,
        commit_filename: req.file_name.unwrap_or_else(|| format!("{:020}.json", req.version)),
        commit_filesize: req.file_size.unwrap_or(0),
        commit_file_modification_timestamp: req.file_modification_timestamp.unwrap_or(req.timestamp),
        commit_timestamp: req.timestamp,
        is_backfilled_latest_commit: false,
    };
    DeltaCommitRepo::insert(&state.pool, &row).await?;
