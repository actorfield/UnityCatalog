...
use uc_db::{
    ...
    repos::{model, property, schema},
};
use uc_errors::UcError;
use uc_openapi::catalog::{
    CreateModelVersion, CreateRegisteredModel, FinalizeModelVersion, ListModelVersionsResponse,
    ListRegisteredModelsResponse, ModelVersionInfo, ModelVersionStatus, RegisteredModelInfo,
    UpdateModelVersion, UpdateRegisteredModel,
};
...
pub async fn create_model(
    ...
    Json(req): Json<CreateRegisteredModel>,
) -> Result<Json<RegisteredModelInfo>, UcError> {
    ...
    let model_storage = match req.storage_location {
        ...
        None => match managed_storage::resolve_storage_root(
            &state.pool,
            &req.catalog_name,
            &req.schema_name,
        )
        .await
        {
            Ok(root) => Some(managed_storage::managed_model_location(
                &root, schema.id, id,
            )),
            Err(_) => None,
        },
    };
    let row = RegisteredModelRow {
        id,
        schema_id: schema.id,
        name: req.name.clone(),
        owner: None,
    ...
    let created = model::create_model(&state.pool, &row).await?;
    if let Some(ref props) = req.properties {
        property::replace(&state.pool, id, "registered_model", props).await?;
    }
    if state.auth_enabled {
    ...
    let props = property::get_for_entity(&state.pool, id, "registered_model")
        .await
        .ok();
    Ok(Json(to_model_info(
        created,
        &req.catalog_name,
        &req.schema_name,
        props,
...
pub async fn list_models(
...
) -> Result<Json<ListRegisteredModelsResponse>, UcError> {
    ...
    let registered_models = rows
        .into_iter()
        .filter(|r| visible_ids.contains(&r.id))
        .map(|r| to_model_info(r, cat, sch, None))
        .collect();
    Ok(Json(ListRegisteredModelsResponse {
        registered_models,
...
pub async fn get_model(
    ...
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let row = model::get_model_by_schema_and_name(&state.pool, schema.id, mdl).await?;
    let props = property::get_for_entity(&state.pool, row.id, "registered_model")
        .await
        .ok();
...
pub async fn update_model(
    ...
    Extension(claims): Extension<Arc<UcClaims>>,
    Path(full_name): Path<String>,
    Json(req): Json<UpdateRegisteredModel>,
) -> Result<Json<RegisteredModelInfo>, UcError> {
    let (cat, sch, mdl) = split3(&full_name)?;
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let existing = model::get_model_by_schema_and_name(&state.pool, schema.id, mdl).await?;
    if state.auth_enabled {
        let user = get_user(&state, &claims.sub).await?;
    ...
    let updated = model::get_model_by_schema_and_name(&state.pool, schema.id, effective_name)
        .await
        .unwrap_or_else(|_| existing.clone());
    let props = property::get_for_entity(&state.pool, existing.id, "registered_model")
        .await
        .ok();
...
pub async fn delete_model(
    ...
    property::delete_for_entity(&state.pool, existing.id, "registered_model").await?;
    model::delete_model(&state.pool, existing.id).await?;
    Ok(StatusCode::OK)
}
...
pub async fn create_version(
    State(state): State<AppState>,
    Extension(claims): Extension<Arc<UcClaims>>,
    Json(req): Json<CreateModelVersion>,
) -> Result<Json<ModelVersionInfo>, UcError> {
    let schema = schema::get_by_full_name(&state.pool, &req.catalog_name, &req.schema_name).await?;
    let model =
    ...
    let version_url = model
        .url
        .as_deref()
        .map(|model_loc| managed_storage::managed_model_version_location(model_loc, next_ver));
    let row = ModelVersionRow {
        id,
        registered_model_id: model.id,
        version: Some(next_ver),
        source: req.source.clone(),
        run_id: req.run_id.clone(),
        status: Some("PENDING_REGISTRATION".into()),
        owner: None,
        created_at: Some(now),
        created_by: auth_sub(&state, &claims).map(String::from),
        updated_at: None,
...
pub async fn list_versions(
    State(state): State<AppState>,
    Path(full_name): Path<String>,
) -> Result<Json<ListModelVersionsResponse>, UcError> {
    let (cat, sch, mdl) = split3(&full_name)?;
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
...
pub async fn get_version(
    State(state): State<AppState>,
    Path((full_name, version)): Path<(String, String)>,
) -> Result<Json<ModelVersionInfo>, UcError> {
    let (cat, sch, mdl) = split3(&full_name)?;
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let model = model::get_model_by_schema_and_name(&state.pool, schema.id, mdl).await?;
    let ver: i32 = version
        .parse()
        .map_err(|_| UcError::invalid_argument("version must be an integer"))?;
    let row = model::get_version(&state.pool, model.id, ver).await?;
...
pub async fn update_version(
    ...
    Extension(claims): Extension<Arc<UcClaims>>,
    Path((full_name, version)): Path<(String, String)>,
    Json(req): Json<UpdateModelVersion>,
) -> Result<Json<ModelVersionInfo>, UcError> {
    let (cat, sch, mdl) = split3(&full_name)?;
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let model = model::get_model_by_schema_and_name(&state.pool, schema.id, mdl).await?;
...
pub async fn delete_version(
    ...
    let ver: i32 = version
        .parse()
        .map_err(|_| UcError::invalid_argument("version must be an integer"))?;
    model::delete_version(&state.pool, model.id, ver).await?;
    Ok(StatusCode::OK)
}
...
pub async fn finalize_version(
    State(state): State<AppState>,
    Path((full_name, version)): Path<(String, String)>,
    Json(req): Json<FinalizeModelVersion>,
) -> Result<Json<ModelVersionInfo>, UcError> {
    let (cat, sch, mdl) = split3(&full_name)?;
    let schema = schema::get_by_full_name(&state.pool, cat, sch).await?;
    let model = model::get_model_by_schema_and_name(&state.pool, schema.id, mdl).await?;
...
fn to_version_info(r: ModelVersionRow, cat: &str, sch: &str, mdl: &str) -> ModelVersionInfo {
    ModelVersionInfo {
        model_name: Some(mdl.to_string()),
        catalog_name: Some(cat.to_string()),
        schema_name: Some(sch.to_string()),
        version: r.version.map(|v| v as i64),
        source: r.source,
        run_id: r.run_id,
        status: parse_model_status(r.status.as_deref()),
        storage_location: r.url,
        comment: r.comment,
        created_at: r.created_at,
        created_by: r.created_by,
