...
pub async fn create_user(
    ...
    Ok(Json(UserResource {
        ...
        user_name: row.name.clone(),
        display_name: None,
        emails: None,
        name: None,
        active: Some(row.is_enabled()),
...
) -> Result<Json<UserResourceList>, UcError> {
    let max = params.count.unwrap_or(50) as i64;
    let (rows, _) = user::list(&state.pool, None, max).await?;
    let resources = rows
        .into_iter()
        .map(|r| UserResource {
            id: Some(r.id.to_string()),
            user_name: r.name.clone(),
            display_name: None,
            emails: None,
            name: None,
            active: Some(r.is_enabled()),
...
pub async fn get_user(
    ...
    let uid: Uuid = id
        .parse()
        .map_err(|_| UcError::invalid_argument("invalid user id"))?;
    let row = user::get_by_id(&state.pool, uid).await?;
    Ok(Json(UserResource {
        id: Some(row.id.to_string()),
        user_name: row.name.clone(),
        display_name: None,
...
pub async fn update_user(
    ...
    let uid: Uuid = id
        .parse()
        .map_err(|_| UcError::invalid_argument("invalid user id"))?;
    let state_str = if req.active.unwrap_or(true) {
        "ENABLED"
    } else {
        "DISABLED"
    };
...
pub async fn delete_user(
    ...
    let uid: Uuid = id
        .parse()
        .map_err(|_| UcError::invalid_argument("invalid user id"))?;
    user::delete(&state.pool, uid).await?;
...
pub async fn get_me(
    ...
    let found = match user::get_by_email(&state.pool, &claims.sub).await? {
        Some(u) => Some(u),
        None => user::get_by_external_id(&state.pool, &claims.sub).await?,
    };
    match found {
...
pub async fn patch_me(
    ...
    if let Some(user) = found {
        ...
        .await?;
        return Ok(Json(UserResource {
            id: Some(updated.id.to_string()),
            user_name: updated.name.clone(),
            display_name: None,
            emails: None,
            name: None,
            active: Some(updated.is_enabled()),
