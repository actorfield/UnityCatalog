...
async fn credential_create_and_get() {
    ...
    let (s, body) = post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("c1", "arn:aws:iam::123:role/r"),
    )
    .await;
...
async fn credential_list() {
    ...
    post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("cl1", "arn:aws:iam::1:role/a"),
    )
    .await;
    post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("cl2", "arn:aws:iam::1:role/b"),
    )
    .await;
...
async fn credential_update_role_arn() {
    ...
    post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("upd_c", "arn:old"),
    )
    .await;
    let (s, _) = patch(
        &app,
        &format!("{UC}/credentials/upd_c"),
        json!({"aws_iam_role":{"role_arn":"arn:new"}}),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
...
async fn credential_update_rename() {
    ...
    post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("ren_c", "arn:x"),
    )
    .await;
    let (s, _) = patch(
        &app,
        &format!("{UC}/credentials/ren_c"),
        json!({"new_name":"ren_c2"}),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
...
async fn credential_duplicate_rejected() {
    ...
    post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("dup_c", "arn:a"),
    )
    .await;
    let (s, _) = post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("dup_c", "arn:b"),
    )
    .await;
...
async fn external_location_crud_and_credential_name() {
    ...
    assert_eq!(s, StatusCode::OK);
    assert_eq!(
        fetched["credential_name"].as_str().unwrap(),
        "el_cred",
        "GET must return credential_name, got: {}",
        fetched["credential_name"]
    );

    // LIST returns credential_name
    let (_, list) = get(&app, &format!("{UC}/external-locations")).await;
    let matching = list["external_locations"]
        .as_array()
        .unwrap()
        .iter()
        .find(|e| e["name"] == "el1")
    ...
    post(
        &app,
        &format!("{UC}/credentials"),
        make_cred("el_cred2", "arn:el2"),
    )
    .await;
    let (s, _) = patch(
        &app,
        &format!("{UC}/external-locations/el1"),
        json!({"credential_name":"el_cred2"}),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
...
async fn permissions_patch_unknown_privilege_returns_400() {
    ...
    post(
        ...
        json!({"userName":"perm_user@x.com","active":true}),
    )
    .await;
    let (s, body) = patch(
        &app,
        &format!("{UC}/permissions/catalog/permp_cat"),
        json!({
