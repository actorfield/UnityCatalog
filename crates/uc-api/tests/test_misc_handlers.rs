...
async fn setup(app: &axum::Router) {
    ...
    post(app, &format!("{UC}/schemas"), json!({"name":"misc_sch","catalog_name":"misc_cat"})).await;
}
...
async fn temp_table_credentials_local_path() {
    ...
    // Create a table
    let (_, tbl) = post(&app, &format!("{UC}/tables"), json!({
    ...
    let (s, creds) = post(&app, &format!("{UC}/temporary-table-credentials"), json!({
        "table_id": table_id,
        "operation": "READ"
    })).await;
...
async fn temp_volume_credentials_local_path() {
    ...
    let (_, vol) = post(&app, &format!("{UC}/volumes"), json!({
    ...
    let (s, _) = post(&app, &format!("{UC}/temporary-volume-credentials"), json!({
        "volume_id": volume_id,
...
async fn temp_path_credentials_local_returns_empty() {
    let (app, _) = build_test_app().await;
    let (s, creds) = post(&app, &format!("{UC}/temporary-path-credentials"), json!({
...
async fn temp_model_version_credentials() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    post(&app, &format!("{UC}/models"), json!({
        "name":"cred_mdl","catalog_name":"misc_cat","schema_name":"misc_sch"
    })).await;
    post(&app, &format!("{UC}/models/versions"), json!({
        "model_name":"cred_mdl","catalog_name":"misc_cat","schema_name":"misc_sch",
        "source":"s3://ml/run1"
    })).await;

    let (s, _) = post(&app, &format!("{UC}/temporary-model-version-credentials"), json!({
...
async fn delta_commits_get_and_post() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    post(&app, &format!("{UC}/tables"), json!({
    ...
    // GET with no commits
    let (s, body) = get(&app, &format!("{UC}/delta/preview/commits?table_full_name=misc_cat.misc_sch.commits_t")).await;
    assert_eq!(s, StatusCode::OK);
    ...
    // POST a commit
    let (s, body) = post(&app, &format!("{UC}/delta/preview/commits"), json!({
    ...
    let (_, after) = get(&app, &format!("{UC}/delta/preview/commits?table_full_name=misc_cat.misc_sch.commits_t")).await;
...
async fn delta_table_credentials_local() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    post(&app, &format!("{DELTA}/catalogs/misc_cat/schemas/misc_sch/tables"), json!({
        "name":"dc_t","location":"/tmp/dc","table-type":"EXTERNAL"
    })).await;

    let (s, body) = get(&app, &format!("{DELTA}/catalogs/misc_cat/schemas/misc_sch/tables/dc_t/credentials")).await;
    assert_eq!(s, StatusCode::OK);
...
async fn delta_path_credentials_local() {
    let (app, _) = build_test_app().await;
    let (s, body) = get(&app, &format!("{DELTA}/temporary-path-credentials?path=file:///tmp/test")).await;
    assert_eq!(s, StatusCode::OK);
    assert!(body["storage-credentials"].is_array());
}
...
async fn delta_staging_credentials() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    let (_, staging) = post(&app, &format!("{DELTA}/catalogs/misc_cat/schemas/misc_sch/staging-tables"),
        json!({"name":"stg_cred"})).await;
    let table_id = staging["table-id"].as_str().unwrap();

    let (s, body) = get(&app, &format!("{DELTA}/staging-tables/{table_id}/credentials")).await;
    assert_eq!(s, StatusCode::OK);
    assert!(body["storage-credentials"].is_array());
}
...
async fn catalog_error_has_uc_error_code_format() {
    ...
    assert!(body["error_code"].as_str().is_some(), "UC errors must have error_code field");
...
async fn delta_error_has_delta_error_type_format() {
    ...
    let (s, body) = get(&app, &format!("{DELTA}/catalogs/misc_cat/schemas/misc_sch/tables/nonexistent")).await;
    assert_eq!(s, StatusCode::NOT_FOUND);
