...
async fn setup(app: &axum::Router) {
    ...
    post(app, &format!("{UC}/schemas"), json!({"name":"d_sch","catalog_name":"d_cat"})).await;
}
...
async fn delta_create_and_load_table() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    let (s, body) = post(&app, &format!("{DELTA}/catalogs/d_cat/schemas/d_sch/tables"), json!({
...
async fn delta_table_exists_head() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    post(&app, &format!("{DELTA}/catalogs/d_cat/schemas/d_sch/tables"), json!({
...
async fn delta_assert_table_uuid_mismatch_returns_409() {
    ...
    let (s, _) = post(&app, &delta_tables("req_t"), json!({
...
async fn delta_assert_table_uuid_match_succeeds() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    let (_, created) = post(&app, &format!("{DELTA}/catalogs/d_cat/schemas/d_sch/tables"), json!({
        "name":"req_ok_t","location":"s3://b/req_ok","table-type":"EXTERNAL"
    })).await;
    let table_uuid = created["metadata"]["table-uuid"].as_str().unwrap().to_string();

    let (s, _) = post(&app, &delta_tables("req_ok_t"), json!({
...
async fn delta_set_properties_update() {
    ...
    let (s, _) = post(&app, &delta_tables("props_t"), json!({"updates":[
...
async fn delta_set_table_comment_update() {
    ...
    let (s, _) = post(&app, &delta_tables("comm_t"), json!({"updates":[
...
async fn delta_rename_table() {
    ...
    let (s, _) = post(&app, &delta_tables("ren_src/rename"), json!({"new-name":"ren_dst"})).await;
    assert_eq!(s, StatusCode::OK);

    let req = axum::http::Request::builder()
        .method("GET").uri(&delta_tables("ren_src"))
        .body(axum::body::Body::empty()).unwrap();
    let res = app.clone().oneshot(req).await.unwrap();
...
async fn delta_delete_table() {
    ...
    let req = axum::http::Request::builder()
        .method("DELETE").uri(&delta_tables("del_dt"))
        .body(axum::body::Body::empty()).unwrap();
    let res = app.clone().oneshot(req).await.unwrap();
...
async fn delta_staging_table_create() {
    let (app, _) = build_test_app().await;
    setup(&app).await;
    let (s, body) = post(&app, &format!("{DELTA}/catalogs/d_cat/schemas/d_sch/staging-tables"),
        json!({"name":"stg1"})).await;
    assert_eq!(s, StatusCode::OK);
