...
async fn scim2_user_create_list_get_delete() {
    ...
    // Create
    let (s, user) = post(&app, &format!("{CTRL}/scim2/Users"),
        json!({"userName":"alice@test.com","active":true})).await;
    ...
    assert_eq!(s, StatusCode::OK);
    assert!(list["Resources"].as_array().unwrap().iter().any(|u| u["userName"] == "alice@test.com"));
    ...
    let req = axum::http::Request::builder()
        ...
        .body(axum::body::Body::from(serde_json::to_vec(&json!({"userName":"alice_new@test.com","active":true})).unwrap()))
        .unwrap();
    ...
    let req3 = axum::http::Request::builder()
        .method("DELETE").uri(&format!("{CTRL}/scim2/Users/{uid}"))
...
async fn auth_token_exchange_returns_jwt() {
    let (app, _) = build_test_app().await;
    let (s, body) = post(&app, &format!("{CTRL}/auth/tokens"), json!({
...
async fn auth_token_wrong_grant_type_returns_400() {
    let (app, _) = build_test_app().await;
    let (s, _) = post(&app, &format!("{CTRL}/auth/tokens"), json!({
        "grant_type": "password",
