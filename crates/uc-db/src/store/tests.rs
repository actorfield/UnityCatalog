...
/// The failure this exists to prevent: two replicas booting together each
/// generating their own key material and each persisting it, so tokens signed
/// by one are rejected by the other depending on which pod served the request.
#[tokio::test]
async fn racing_replicas_agree_on_one_singleton_value() {
    let log = Arc::new(MemLog::default());
    let a = Store::open(log.clone()).await.unwrap();
    let b = Store::open(log.clone()).await.unwrap();

    let from_a = a
        .get_or_create_object("_uc_log/_keys.json", || Ok(b"key-from-a".to_vec()))
        .await
        .unwrap();
    let from_b = b
        .get_or_create_object("_uc_log/_keys.json", || Ok(b"key-from-b".to_vec()))
        .await
        .unwrap();

    assert_eq!(from_a, b"key-from-a".to_vec());
    assert_eq!(
        from_b, from_a,
        "the loser must adopt the winner's value, not persist its own"
    );

    // And a replica that arrives later reads the same thing.
    let c = Store::open(log).await.unwrap();
    let from_c = c
        .get_or_create_object("_uc_log/_keys.json", || Ok(b"key-from-c".to_vec()))
        .await
        .unwrap();
    assert_eq!(from_c, from_a);
}

#[tokio::test]
async fn a_singleton_is_not_regenerated_across_restarts() {
    let log = Arc::new(MemLog::default());
    let first = Store::open(log.clone()).await.unwrap();
    let original = first
        .get_or_create_object("_uc_log/_keys.json", || Ok(b"generated-once".to_vec()))
        .await
        .unwrap();

    // Restart: same log, fresh process. Regenerating here is exactly the bug
    // that invalidates every issued token.
    let restarted = Store::open(log).await.unwrap();
    let after = restarted
        .get_or_create_object("_uc_log/_keys.json", || {
            panic!("must not regenerate when the object already exists")
        })
        .await
        .unwrap();
...
#[tokio::test]
async fn singleton_creation_does_not_touch_the_metastore_log() {
    let log = Arc::new(MemLog::default());
    let store = Store::open(log.clone()).await.unwrap();
    store
        .get_or_create_object("_uc_log/_keys.json", || Ok(b"x".to_vec()))
        .await
        .unwrap();

    assert_eq!(store.snapshot().await.version, 0);
    // And it does not confuse replay: _keys.json sits under the log prefix.
    let cold = Store::open(log).await.unwrap();
    assert_eq!(cold.snapshot().await.version, 0);
}
