...
use crate::{models::table::{ColumnRow, TableRow}, pool::AnyPool};
...
impl TableRepo {
    pub async fn create(pool: &AnyPool, row: &TableRow) -> Result<TableRow, UcError> {
        ...
        .bind(row.id).bind(row.schema_id).bind(&row.name).bind(&row.r#type)
        .bind(&row.owner).bind(row.created_at).bind(&row.created_by)
        .bind(&row.data_source_format).bind(&row.comment).bind(&row.url)
        .bind(row.column_count).bind(&row.view_definition)
        .fetch_one(pool)
        .await
        .map_err(|e| match e {
            sqlx::Error::Database(ref db) if db.is_unique_violation() =>
                UcError::new(ErrorCode::TableAlreadyExists, format!("Table '{}' already exists", row.name)),
    ...
    pub async fn get_by_id(pool: &AnyPool, id: Uuid) -> Result<TableRow, UcError> {
        sqlx::query_as::<_, TableRow>("SELECT * FROM uc_tables WHERE id = $1")
            ...
            .map_err(|e| match e {
                sqlx::Error::RowNotFound => UcError::new(ErrorCode::TableNotFound, format!("Table '{}' not found", id)),
                other => crate::sqlx_err(other),
            })
    }

    pub async fn get_by_schema_and_name(
        pool: &AnyPool, schema_id: Uuid, name: &str,
    ) -> Result<TableRow, UcError> {
        sqlx::query_as::<_, TableRow>(
            "SELECT * FROM uc_tables WHERE schema_id = $1 AND name = $2",
        )
        .bind(schema_id).bind(name).fetch_one(pool).await
        .map_err(|e| match e {
            sqlx::Error::RowNotFound => UcError::new(ErrorCode::TableNotFound, format!("Table '{}' not found", name)),
            other => crate::sqlx_err(other),
        })
    }
    ...
    pub async fn list(
        ...
        let rows: Vec<TableRow> = if let Some(token) = page_token {
            ...
            .bind(schema_id).bind(token).bind(max_results + 1).fetch_all(pool).await.map_err(crate::sqlx_err)?
        } else {
            sqlx::query_as::<_, TableRow>(
    ...
    pub async fn delete(pool: &AnyPool, id: Uuid) -> Result<(), UcError> {
        let r = sqlx::query("DELETE FROM uc_tables WHERE id = $1").bind(id).execute(pool).await.map_err(crate::sqlx_err)?;
        if r.rows_affected() == 0 {
            return Err(UcError::new(ErrorCode::TableNotFound, format!("Table '{}' not found", id)));
        }
        Ok(())
    }
    ...
    pub async fn insert_columns(pool: &AnyPool, columns: &[ColumnRow]) -> Result<(), UcError> {
        for col in columns {
            ...
            .bind(col.id).bind(col.table_id).bind(&col.name).bind(col.ordinal_position)
            .bind(&col.type_text).bind(&col.type_json).bind(&col.type_name)
            .bind(col.type_precision).bind(col.type_scale).bind(&col.type_interval_type)
            .bind(col.nullable).bind(&col.comment).bind(col.partition_index)
