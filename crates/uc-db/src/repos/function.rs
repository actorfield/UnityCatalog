...
pub async fn get_by_schema_and_name(
    ...
    sqlx::query_as::<_, FunctionRow>("SELECT * FROM uc_functions WHERE schema_id=$1 AND name=$2")
        .bind(schema_id)
        .bind(name)
        .fetch_one(pool)
        .await
...
pub async fn delete(pool: &AnyPool, id: Uuid) -> Result<(), UcError> {
    sqlx::query("DELETE FROM uc_function_parameters WHERE function_id=$1")
        .bind(id)
        .execute(pool)
        .await
        .map_err(crate::sqlx_err)?;
    let r = sqlx::query("DELETE FROM uc_functions WHERE id=$1")
        .bind(id)
        .execute(pool)
        .await
        .map_err(crate::sqlx_err)?;
