...
impl ExternalLocationRepo {
    pub async fn create(pool: &AnyPool, row: &ExternalLocationRow) -> Result<ExternalLocationRow, UcError> {
    ...
    pub async fn get_by_name(pool: &AnyPool, name: &str) -> Result<ExternalLocationRow, UcError> {
        sqlx::query_as::<_, ExternalLocationRow>("SELECT * FROM uc_external_locations WHERE name=$1")
            ...
            .map_err(|e| match e {
                sqlx::Error::RowNotFound => UcError::new(ErrorCode::NotFound, format!("External location '{}' not found", name)),
                other => crate::sqlx_err(other),
            })
    }
    ...
    pub async fn delete(pool: &AnyPool, name: &str) -> Result<(), UcError> {
        let r = sqlx::query("DELETE FROM uc_external_locations WHERE name=$1").bind(name).execute(pool).await.map_err(crate::sqlx_err)?;
        if r.rows_affected()==0 { return Err(UcError::new(ErrorCode::NotFound, format!("External location '{}' not found", name))); }
        Ok(())
    }

    /// Find the external location whose url is a prefix of the given path.
    pub async fn find_by_path_prefix(pool: &AnyPool, path: &str) -> Result<Option<ExternalLocationRow>, UcError> {
