...
impl SchemaRepo {
    ...
    pub async fn update(
        ...
        .bind(new_name).bind(comment).bind(owner)
        .bind(updated_at).bind(updated_by).bind(id)
        .fetch_one(pool)
        .await
    ...
    pub async fn delete(pool: &AnyPool, id: Uuid) -> Result<(), UcError> {
        ...
        if result.rows_affected() == 0 {
            return Err(UcError::new(ErrorCode::SchemaNotFound, format!("Schema '{}' not found", id)));
        }
        Ok(())
    }
}
