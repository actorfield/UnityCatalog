...
pub async fn get_by_name(pool: &AnyPool, name: &str) -> Result<UserRow, UcError> {
    sqlx::query_as::<_, UserRow>("SELECT * FROM uc_users WHERE name = $1")
        ...
        .map_err(|e| match e {
            sqlx::Error::RowNotFound => {
                UcError::new(ErrorCode::NotFound, format!("User '{}' not found", name))
            }
            other => crate::sqlx_err(other),
        })
}
...
pub async fn find_or_create_by_external_id(
    ...
    if let Some(row) = get_by_external_id(pool, external_id).await? {
        return Ok(row);
    }
    let id = Uuid::now_v7();
...
pub async fn list(
    ...
    let rows: Vec<UserRow> = if let Some(token) = page_token {
        ...
        sqlx::query_as::<_, UserRow>("SELECT * FROM uc_users ORDER BY name LIMIT $1")
            .bind(max_results + 1)
            .fetch_all(pool)
            .await
            .map_err(crate::sqlx_err)?
